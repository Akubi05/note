...
算法评价指标
为了在不同的数据集上对各种竞争方法进行基准测试，采用以下六个评估指标来量化聚类结果的一致性和消除批次效应的能力。
调整随机数指数 (ARI) 用于量化聚类准确率，可以通过Python模块sklearn.metrics.cluster中的adjusted_rand_score函数计算。ARI衡量两个聚类结果之间的相似性，定义为:
ARI=
其中n_ij表示聚类结果中属于簇i的细胞数，也表示真实细胞类型标签中属于细胞类型j的细胞数；a_i表示簇i的细胞数；b_j表示细胞类型j的细胞数；n表示细胞总数。我们计算ARI来比较整合数据的聚类结果与预定义的细胞类型。ARI的取值范围为[0,1]，值越大表示相似度越高。

2

NMI
归一化互信息 (NMI) 也用于衡量聚类准确率，可以通过 Python 模块 sklearn.metrics.cluster 中的 normalized_mutual_info_score 函数计算。NMI 定义为

其中符号与ARI中的相同。NMI取值范围为[0, 1]，值越高，聚类结果与真实细胞类型的相似度也就越高。类结果与真实细胞类型的相似度也就越高。
3.
ASW_celltype
细胞类型平均轮廓宽度 (ASW_celltype) 用于评估细胞类型纯度，可通过 Python 模块 sklearn.metrics 中的函数 Silhouette_score 计算。细胞 i 的细胞类型标签轮廓宽度定义为

其中 ai 是细胞 i 到所有具有相同标签的细胞的平均距离，bi 是细胞 i 到每组被分配不同标签的细胞的最小平均距离。ASW_celltype 是所有细胞轮廓宽度的平均值，取值范围为 [0, 1]，值越大表示细胞与具有相同标签的细胞越近，与具有不同标签的细胞越远。我们根据低维（默认维度为 32）PCA 嵌入空间中预定义的细胞类型标签来计算 ASW_celltype。

4.
ASW_batch
批次平均轮廓宽度 (ASW_batch) 用于评估批次的全局混合程度。第 i 个细胞批次的轮廓宽度定义为

其中 ai 是细胞 i 到同一批次所有细胞的平均距离，bi 是细胞 i 到分配到其他批次的每组细胞的最小平均距离。ASW_batch 是所有细胞轮廓宽度的平均值，值越高表示细胞与同一批次的细胞越接近，与不同批次的细胞越远。我们推断，ASW_batch 值越高意味着批次间的互斥性越强，反之，ASW_batch 值越低通常表示批次混合效果更好，批次效应校正也更佳。ASW_batch 的值也在 [0, 1] 范围内。

5.
iLISI
逆辛普森积分指数 (iLISI) 用于评估积分后批次的局部混合效果。局部逆辛普森指数 (LISI) 可用于测量批次分布 (iLISI)，该指数基于预先选定的困惑度 (perplexity) 选择的局部邻居。然后，使用选定的细胞邻居，根据 iLISI 指数的批次标签计算 LISI，接近预期批次数的分数表示批次混合效果良好。我们使用 R 包 lisi 中的 compute_lisi 函数为数据集中的所有细胞计算 iLISI，输出平均分数，iLISI 越高，表示批次混合效果越好。

6.
BatchKL
我们使用 KL 散度来评估各种单细胞聚类算法在消除批次效应方面的性能，即评估不同批次的细胞在每个聚类中混合的随机性。B个不同批次的批次混合的 KL 散度计算如下：

我们使用上面定义的区域混合 KL 散度，从所有批次中随机选择 100 个细胞，计算批次混合的 KL 散度。每个批次的细胞区域比例是根据每个随机选择的细胞的 K 个最近邻细胞集合计算的（K 可以根据批次数量进行不同的设置，我们建议 K = 5 × 批次数量）。最终的 KL 散度计算为区域 KL 散度的平均值。我们使用不同的随机选择的细胞重复此过程 200 次，以生成最终 KL 散度的箱线图。最终 KL 散度越小，表示批次混合效果越好，即在消除批次效应方面越有效。
